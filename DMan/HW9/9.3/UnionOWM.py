...
def get_weather():
    ...
    if detect_language[0] == 'uk':
        weather_info = (f"Хманість y місті {city}\n {w.detailed_status}\n"
                        f"Температура дорівнює {round(w.temperature('celsius')['temp'], 2)}°C"
                        f"\nШвидкicть вітру станове {w.wind()['speed']} км/год\n"
                        f"Вологість повітрю станове {w.humidity}")
    elif detect_language[0] == 'en':
        weather_info = (f"Conditions of {city} city\n: {w.detailed_status}\n"
                    f"Temperature is {round(w.temperature('celsius')['temp'], 2)}"
